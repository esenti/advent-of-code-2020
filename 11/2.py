...
def check_position(board: List[List[str]], x: int, y: int, dx: int, dy: int) -> int:
    if x < 0 or y < 0:
        return 0
    ...
    return 1 if board[y][x] == '#' else 0 if board[y][x] == 'L' else check_position(board, x + dx, y + dy, dx, dy)
...
def count_neighbours(board: List[List[str]], x: int, y: int) -> int:
    ...
    for dx, dy in product([-1, 0, 1], [-1, 0, 1]):
        if not (dx == 0 and dy == 0):
            count += check_position(board, x + dx, y + dy, dx, dy)
    ...
    for y, row in enumerate(board):
        for x, v in enumerate(row):
            if v == 'L' and count_neighbours(board, x, y) == 0:
                result[y][x] = '#'
                changed = True
            elif v == '#' and count_neighbours(board, x, y) >= 5:
                result[y][x] = 'L'
                changed = True
            else:
                result[y][x] = v
